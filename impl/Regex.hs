...
r (RegConcat (RegBackref n:rs)) s begin groups =
  do
    (post,match,groups') <- r (RegConcat rs) s2 (begin && s1 == []) groups
...
r (RegConcat ((RegRep r1 n (Just m)):rs)) s begin groups =
  if n > 0
  then r (RegConcat (r1:(RegRep r1 (n-1) (Just (m-1))):rs)) s begin groups
  else Nothing
...
r (RegConcat ((RegRep r1 n Nothing):rs)) s begin groups =
  if n > 0
  then r (RegConcat (r1:(RegRep r1 (n-1) Nothing):rs)) s begin groups
  else Nothing
r (RegConcat ((RegStar r1):rs)) s begin groups =
  if result /= Nothing
  then result
